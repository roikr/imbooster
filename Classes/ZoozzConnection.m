		...
		switch (requestType) {
			...
			case ZoozzAuthenticateTrial: {
				...
				if (requestType == ZoozzLogin) {
					[request setAllHTTPHeaderFields:[ZoozzConnection requestLoginHeaderWithSessionID:appDelegate.localStorage.sessionID withAPNToken:appDelegate.APNToken]];
				}
				else {
			...
			case ZoozzLibrary: {
				...
				NSMutableDictionary * dict = [NSMutableDictionary dictionaryWithDictionary:[ZoozzConnection requestHeaderWithSessionID:appDelegate.localStorage.sessionID]];
				IminentAppDelegate *appDelegate = (IminentAppDelegate *)[[UIApplication sharedApplication] delegate];
				NSString * date = appDelegate.localStorage.libraryDate;
				if (date) 
			...
			if (!appDelegate.bLoggedIn) {
				...
				for (NSHTTPCookie * cookie in arr) {
					...
					if ([cookie.name isEqualToString:@"sid"]) {
						appDelegate.localStorage.sessionID=[cookie value];
						appDelegate.localStorage.tokenNumber = 1;
